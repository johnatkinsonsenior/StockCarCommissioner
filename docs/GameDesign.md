...
**Days 123–126** package that office for playtesters: a writable-book
loop, a save/load and era-rewind bug pass, a zip with `KNOWN_ISSUES.md`,
and a tester career that keeps Superbirds and pulled plates. **Days
127–133**
...
checklist. Python still owns the career world. Main menu item 7 (or `python3 prototype/run_ui.py`, `./play_ui.sh`, or Windows `StockCarCommissioner.exe`) writes `godot/data/ui_snapshot.json` and opens the editor. On Windows the exe starts `launch_office.cmd`; a zip built with `--windows-runtime` already has Godot 4.4 and Python under `tools/`, otherwise first play downloads them. `play_ui` Continues `saves/office.json` when that slot exists. Testers unpack `dist/stock-car-commissioner-0.6.0-cup.zip` (`python3 prototype/package_alpha.py`) and double-click the exe; Git is not required. See `PLAYTEST.md` and `KNOWN_ISSUES.md`.
